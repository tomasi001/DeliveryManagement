  ...
  try {
    ...
    if (session.client_email && delivered.length > 0) {
      await sendMail({
        ...
        html: getDeliveryConfirmationTemplate(session.client_name, session.address, delivered),
    ...
    await sendMail({
      ...
      html: getDeliveryReportTemplate(session.client_name, session.address, delivered, returned),
